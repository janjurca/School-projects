...
void append_cluster(struct cluster_t *c, struct obj_t obj)
{
    ...
    c->obj[c->size] = obj;// pridej na konec
    c->size = c->size + 1;
    // TO-DO
}
/*
...
float obj_distance(struct obj_t *o1, struct obj_t *o2)
{
    ...
    float vzdalenost = sqrt(sirka * sirka + vyska * vyska);
...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    for(int i = 0; i < c1->size; i++){
        for(int j = 0; j < c2->size; j++){
            ...
            if(vzdalenost < tmp_distance){
                vzdalenost = tmp_distance;
            }
        }
    }
    return vzdalenost;
...
void find_neighbours(struct cluster_t *carr, int narr, int *c1, int *c2)
{
    ...
    for(int i = 0; i < narr; i++){
        for(int j = 0; j < narr; j++){
            if(i == j)
                continue;
            float tmp_dst = cluster_distance(&carr[i],&carr[j]);
            if(tmp_dst < vzdalenost){
                vzdalenost = tmp_dst;
                *c1 = i;
                *c2 = j;
            }
...
int load_clusters(char *filename, struct cluster_t **arr)
{
    ...
    if (NULL == file)
    {
         fprintf(stderr,"Nelze otevřít soubor.\n");
         *arr = NULL;
         return (-1);
    }
    int id = 0;
    ...
    int count = 0;
    int j = 0;
    if(EOF != fscanf(file, "%30[^\n]\n", buffer)){
        sscanf(buffer,"count=%d",&count);
        if(count < 1){
            fprintf(stderr,"Špatný formát souboru, chybí počet řádků.\n");
            return -1;
        }
        free(*arr);
    ...
    for(int i = 0; EOF != fscanf(file, "%30[^\n]\n", buffer) && i < count;i++)
    {
         j++;
...
int main(int argc, char *argv[])
{
    ...
    if(argc > 1 && argc < 4){
        ...
        if(count < 0){
            fprintf(stderr,"chyba pri nacitani clusteru");
            return -1;
        }
        if(argc == 3){
            if(atoi(argv[2]) > 1)
                ...
                fprintf(stderr,"chybna hodnota poctu clustru\n");
                return -1;
            }
        }
        int id1 = 0;
        ...
        fprintf(stderr,"Nespravne zadane argumenty\n"
                       "syntaxe spusteni ./proj3 SOUBOR [N] - N pocet shluku \n");
    }
    // TO-DO
}
